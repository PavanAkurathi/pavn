import { ImageResponse } from 'next/og'

export const runtime = 'edge'

// Image metadata
export const alt = 'Workers Hive - Smart Staff Scheduling'
export const size = {
    width: 1200,
    height: 630,
}

export const contentType = 'image/png'

// Image generation
export default async function Image() {

    return new ImageResponse(
        (
            // ImageResponse JSX element
            <div
                style={{
                    fontSize: 128,
                    background: 'linear-gradient(to bottom right, #0f172a, #1e293b)',
                    width: '100%',
                    height: '100%',
                    display: 'flex',
                    flexDirection: 'column',
                    alignItems: 'center',
                    justifyContent: 'center',
                    color: 'white',
                    fontFamily: 'sans-serif',
                }}
            >
                <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
                    {/* Simple Logo Icon representation */}
                    <div style={{
                        display: 'flex',
                        padding: '10px',
                        background: 'white',
                        borderRadius: '20px',
                        color: '#0f172a',
                        fontSize: '80px',
                        fontWeight: 'bold'
                    }}>
                        WH
                    </div>
                    <div style={{ fontSize: 100, fontWeight: 'bold', letterSpacing: '-0.05em' }}>Workers Hive</div>
                </div>
                <div style={{ fontSize: 40, marginTop: '40px', color: '#94a3b8' }}>
                    Smart Staff Scheduling for Hospitality
                </div>
            </div>
        ),
        // ImageResponse options
        {
            // For convenience, we can re-use the exported opengraph-image size config to also set the ImageResponse's width and height.
            ...size,
        }
    )
}
