-- PostGIS is required by location_position_gist_idx, which indexes
-- ST_SetSRID(ST_MakePoint(...))::geography over the jsonb `position` column.
-- drizzle-kit does not emit extensions from schema.ts, so it is declared here.
-- Without it a fresh database cannot build this baseline at all.
